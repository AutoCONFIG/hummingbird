...
import (
	...
	"github.com/winc-link/hummingbird/internal/tools/sqldb/postgres"
	"gorm.io/gorm"
)
...
func deviceLibrariesSearch(c *Client, offset int, limit int, req dtos.DeviceLibrarySearchQueryRequest) (deviceLibraries []models.DeviceLibrary, count uint32, edgeXErr error) {
	...
	tx = postgres.BuildCommonCondition(tx, dl, req.BaseSearchConditionQuery)
	// 特殊条件
	...
	if req.NameAliasLike != "" {
		tx = tx.Where(`"name" LIKE ? OR "alias" LIKE ? OR "description" LIKE ?`, postgres.MakeLikeParams(req.NameAliasLike), postgres.MakeLikeParams(req.NameAliasLike), postgres.MakeLikeParams(req.NameAliasLike))
	}
	if req.NoInIds != "" {
