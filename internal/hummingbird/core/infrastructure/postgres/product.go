...
func productsSearch(c *Client, offset int, limit int, preload bool, req dtos.ProductSearchQueryRequest) (products []models.Product, count uint32, edgeXErr error) {
	...
	tx = postgres.BuildCommonCondition(tx, dp, req.BaseSearchConditionQuery)

	if req.Name != "" {
		tx = tx.Where(`"name" LIKE ?`, postgres.MakeLikeParams(req.Name))
	}
...
func batchUpsertProduct(c *Client, d []models.Product) (int64, error) {
	...
	tx := c.Pool.Session(&gorm.Session{FullSaveAssociations: true}).Clauses(
		clause.OnConflict{
			UpdateAll: true,
		}).CreateInBatches(d, postgres.CreateBatchSize)
	num := tx.RowsAffected
	err := tx.Error
	if err != nil {
		return num, err
...
func batchInsertSystemProperties(c *Client, p []models.Properties) (int64, error) {
	...
	tx := c.Pool.CreateInBatches(p, postgres.CreateBatchSize)
	num := tx.RowsAffected
	err := tx.Error
	if err != nil {
		return num, err
