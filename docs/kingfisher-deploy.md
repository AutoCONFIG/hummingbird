...
### 3.1 实测结论（2026-09-13，单机 docker 环境）

| 项 | 结果 |
|---|---|
| mqtt-broker:2.6 三元组校验 | **不强制**（任意凭据可连接发布，如 sim/sim）→ 生产必须：broker 限设备出口 IP / 换 EMQX+HTTP 认证 / 上 8883+TLS |
| 500 台注册设备压测 | 500 帧 266ms 发布、TDengine 500/500 全入库（超级表 count）、应用 CPU 1.2%、RSS 104MB、错误 0 |
| 500 台注册耗时 | API 批量注册 4s（含 500 张 TDengine 子表创建） |
| 告警链路延迟 | 低氧帧上报 → eKuiper 触发 → 回调入库 → 报警中心可见 ≈ 2~4s |
